...
class CrazyClearanceAdapter(BaseAdapter):
    SITE_NAME = 'crazyclearance'

    async def search_product(self, page: Page, product_name: str) -> List[Product]:
        products = []
        try:
            search_url = f'https://crazyclearance.uk/?s={product_name.replace(" ", "+")}'
            await page.goto(search_url, wait_until='domcontentloaded', timeout=30000)
            await self.accept_cookies(page)
            await page.wait_for_timeout(2000)
            items = []
            for selector in ['.woocommerce-loop-product', '[class*="product"]', 'article']:
