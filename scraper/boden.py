...
class BodenAdapter(BaseAdapter):
    SITE_NAME = 'boden'

    async def search_product(self, page: Page, product_name: str) -> List[Product]:
        products = []
        try:
            search_url = f'https://www.boden.co.uk/en-gb/search?q={product_name.replace(" ", "+")}'
            await page.goto(search_url, wait_until='domcontentloaded', timeout=30000)
            await self.accept_cookies(page)
            await page.wait_for_timeout(2000)
            items = []
            for selector in ['.product-tile', '[class*="product"]', 'article']:
                ...
                try:
                    ...
                    if product_name.lower() in p_name.lower():
                        ...
                        if product_url and not product_url.startswith('http'):
                            product_url = 'https://www.boden.co.uk' + product_url
